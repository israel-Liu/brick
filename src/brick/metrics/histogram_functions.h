...
#ifndef BRICK_METRICS_HISTOGRAM_FUNCTIONS_H_
#define BRICK_METRICS_HISTOGRAM_FUNCTIONS_H_
...
BRICK_EXPORT void UmaHistogramExactLinear(const std::string& name,
                                         int sample,
                                         int value_max);
...
BRICK_EXPORT void UmaHistogramBoolean(const std::string& name, bool sample);

// For adding histogram with percent.
// Percents are integer between 1 and 100.
// Sample usage:
//   base::UmaHistogramPercentage("My.Percent", 69)
BRICK_EXPORT void UmaHistogramPercentage(const std::string& name, int percent);

// For adding counts histogram.
// Sample usage:
//   base::UmaHistogramCustomCounts("My.Counts", some_value, 1, 600, 30)
BRICK_EXPORT void UmaHistogramCustomCounts(const std::string& name,
                                          int sample,
                                          int min,
                                          int max,
                                          int buckets);
...
BRICK_EXPORT void UmaHistogramCounts100(const std::string& name, int sample);
BRICK_EXPORT void UmaHistogramCounts1000(const std::string& name, int sample);
BRICK_EXPORT void UmaHistogramCounts10000(const std::string& name, int sample);
BRICK_EXPORT void UmaHistogramCounts100000(const std::string& name, int sample);
BRICK_EXPORT void UmaHistogramCounts1M(const std::string& name, int sample);
BRICK_EXPORT void UmaHistogramCounts10M(const std::string& name, int sample);

// For histograms storing times.
BRICK_EXPORT void UmaHistogramCustomTimes(const std::string& name,
                                         TimeDelta sample,
                                         TimeDelta min,
                                         TimeDelta max,
                                         int buckets);
// For short timings from 1 ms up to 10 seconds (50 buckets).
BRICK_EXPORT void UmaHistogramTimes(const std::string& name, TimeDelta sample);
// For medium timings up to 3 minutes (50 buckets).
BRICK_EXPORT void UmaHistogramMediumTimes(const std::string& name,
                                         TimeDelta sample);
// For time intervals up to 1 hr (50 buckets).
BRICK_EXPORT void UmaHistogramLongTimes(const std::string& name,
                                       TimeDelta sample);

// For recording memory related histograms.
// Used to measure common KB-granularity memory stats. Range is up to 500M.
BRICK_EXPORT void UmaHistogramMemoryKB(const std::string& name, int sample);
// Used to measure common MB-granularity memory stats. Range is up to ~1G.
BRICK_EXPORT void UmaHistogramMemoryMB(const std::string& name, int sample);
// Used to measure common MB-granularity memory stats. Range is up to ~64G.
BRICK_EXPORT void UmaHistogramMemoryLargeMB(const std::string& name, int sample);
...
BRICK_EXPORT void UmaHistogramSparse(const std::string& name, int sample);

}  // namespace base

#endif  // BRICK_METRICS_HISTOGRAM_FUNCTIONS_H_
