...
#ifndef BRICK_WIN_WIN_UTIL_H_
#define BRICK_WIN_WIN_UTIL_H_
...
BRICK_EXPORT bool GetUserSidString(std::wstring* user_sid);
...
BRICK_EXPORT bool UserAccountControlIsEnabled();

// Sets the boolean value for a given key in given IPropertyStore.
BRICK_EXPORT bool SetBooleanValueForPropertyStore(
    IPropertyStore* property_store,
    const PROPERTYKEY& property_key,
    bool property_bool_value);

// Sets the string value for a given key in given IPropertyStore.
BRICK_EXPORT bool SetStringValueForPropertyStore(
    IPropertyStore* property_store,
    const PROPERTYKEY& property_key,
    const wchar_t* property_string_value);

// Sets the CLSID value for a given key in a given IPropertyStore.
BRICK_EXPORT bool SetClsidForPropertyStore(IPropertyStore* property_store,
                                          const PROPERTYKEY& property_key,
...
// Win7.
BRICK_EXPORT bool SetAppIdForPropertyStore(IPropertyStore* property_store,
                                          const wchar_t* app_id);

// Adds the specified |command| using the specified |name| to the AutoRun key.
// |root_key| could be HKCU or HKLM or the root of any user hive.
BRICK_EXPORT bool AddCommandToAutoRun(HKEY root_key, const string16& name,
                                     const string16& command);
// Removes the command specified by |name| from the AutoRun key. |root_key|
// could be HKCU or HKLM or the root of any user hive.
BRICK_EXPORT bool RemoveCommandFromAutoRun(HKEY root_key, const string16& name);
...
BRICK_EXPORT bool ReadCommandFromAutoRun(HKEY root_key,
                                        const string16& name,
                                        string16* command);
...
BRICK_EXPORT void SetShouldCrashOnProcessDetach(bool crash);
BRICK_EXPORT bool ShouldCrashOnProcessDetach();

// Adjusts the abort behavior so that crash reports can be generated when the
// process is aborted.
BRICK_EXPORT void SetAbortBehaviorForCrashReporting();

// Checks whether the supplied |hwnd| is in Windows 10 tablet mode. Will return
// false on versions below 10.
BRICK_EXPORT bool IsWindows10TabletMode(HWND hwnd);
...
BRICK_EXPORT bool IsTabletDevice(std::string* reason, HWND hwnd);
...
// This function optionally sets the |reason| parameter to determine as to why
// or why not a device was deemed to be a tablet.
BRICK_EXPORT bool IsDeviceUsedAsATablet(std::string* reason);
...
BRICK_EXPORT bool IsKeyboardPresentOnSlate(std::string* reason, HWND hwnd);
...
BRICK_EXPORT bool IsEnrolledToDomain();

// Returns true if the machine is being managed by an MDM system.
BRICK_EXPORT bool IsDeviceRegisteredWithManagement();
...
BRICK_EXPORT bool IsUser32AndGdi32Available();
...
BRICK_EXPORT bool GetLoadedModulesSnapshot(HANDLE process,
...
BRICK_EXPORT void EnableFlicks(HWND hwnd);
BRICK_EXPORT void DisableFlicks(HWND hwnd);

// Returns true if the process is per monitor DPI aware.
BRICK_EXPORT bool IsProcessPerMonitorDpiAware();

// Enable high-DPI support for the current process.
BRICK_EXPORT void EnableHighDPISupport();

}  // namespace win
}  // namespace base

#endif  // BRICK_WIN_WIN_UTIL_H_
