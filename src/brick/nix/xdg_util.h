...
#ifndef BRICK_NIX_XDG_UTIL_H_
#define BRICK_NIX_XDG_UTIL_H_
...
BRICK_EXPORT extern const char kDotConfigDir[];

// The XDG config directory environment variable.
BRICK_EXPORT extern const char kXdgConfigHomeEnvVar[];
...
BRICK_EXPORT FilePath GetXDGDirectory(Environment* env, const char* env_name,
                                     const char* fallback_dir);

// Wrapper around xdg_user_dir_lookup() from src/brick/third_party/xdg-user-dirs
// This looks up "well known" user directories like the desktop and music
// folder. Examples of |dir_name| are DESKTOP and MUSIC.
BRICK_EXPORT FilePath GetXDGUserDirectory(const char* dir_name,
                                         const char* fallback_dir);
...
BRICK_EXPORT DesktopEnvironment GetDesktopEnvironment(Environment* env);

// Return a string representation of the given desktop environment.
// May return NULL in the case of DESKTOP_ENVIRONMENT_OTHER.
BRICK_EXPORT const char* GetDesktopEnvironmentName(DesktopEnvironment env);
// Convenience wrapper that calls GetDesktopEnvironment() first.
BRICK_EXPORT const char* GetDesktopEnvironmentName(Environment* env);

}  // namespace nix
}  // namespace base

#endif  // BRICK_NIX_XDG_UTIL_H_
