...
#ifndef BRICK_I18N_TIME_FORMATTING_H_
#define BRICK_I18N_TIME_FORMATTING_H_
...
BRICK_I18N_EXPORT string16 TimeFormatTimeOfDay(const Time& time);

// Returns the time of day in 24-hour clock format with millisecond accuracy,
// e.g., "15:07:30.568"
BRICK_I18N_EXPORT string16 TimeFormatTimeOfDayWithMilliseconds(const Time& time);
...
BRICK_I18N_EXPORT string16 TimeFormatTimeOfDayWithHourClockType(
    const Time& time,
    HourClockType type,
    AmPmClockType ampm);

// Returns a shortened date, e.g. "Nov 7, 2007"
BRICK_I18N_EXPORT string16 TimeFormatShortDate(const Time& time);

// Returns a numeric date such as 12/13/52.
BRICK_I18N_EXPORT string16 TimeFormatShortDateNumeric(const Time& time);

// Returns a numeric date and time such as "12/13/52 2:44:30 PM".
BRICK_I18N_EXPORT string16 TimeFormatShortDateAndTime(const Time& time);

// Returns a month and year, e.g. "November 2007"
BRICK_I18N_EXPORT string16 TimeFormatMonthAndYear(const Time& time);

// Returns a numeric date and time with time zone such as
// "12/13/52 2:44:30 PM PST".
BRICK_I18N_EXPORT string16
...
BRICK_I18N_EXPORT string16 TimeFormatFriendlyDateAndTime(const Time& time);

// Formats a time in a friendly sentence format, e.g.
// "Monday, March 6, 2008".
BRICK_I18N_EXPORT string16 TimeFormatFriendlyDate(const Time& time);
...
BRICK_I18N_EXPORT string16 TimeFormatWithPattern(const Time& time,
                                                const char* pattern);
...
BRICK_I18N_EXPORT bool TimeDurationFormat(const TimeDelta time,
...
// TODO(chengx): fix function output when width = DURATION_WIDTH_NUMERIC
// (http://crbug.com/675791)
BRICK_I18N_EXPORT bool TimeDurationFormatWithSeconds(
...
BRICK_I18N_EXPORT string16 DateIntervalFormat(const Time& begin_time,
                                             const Time& end_time,
                                             DateFormat format);
...
// k12HourClock (en-US).
// k24HourClock (en-GB).
BRICK_I18N_EXPORT HourClockType GetHourClockType();

}  // namespace base

#endif  // BRICK_I18N_TIME_FORMATTING_H_
